...
class NN:
    ...
    def compute_gradients(self, X, Y):
        ...
        for idx in range(num_layers-2, -1, -1):
            ...
            grad = (np.matmul(ws[idx+1].T, grad))*self.activation_function.backprop_grad(np.matmul(ws[idx],D_stack[idx]) + bs[idx])
...
class SquaredLoss(LossFunction):
    def loss(self, Y, Yhat):
        ...
        in Yhat; compute the loss associated with these predictions.
        """
        #eturn (1.0/(2*Y.shape[1]))*(np.linalg.norm(Yhat - Y)**2)
        sum = (np.linalg.norm(Yhat-Y)**2)
        sum = sum/(2*Y.shape[1])
